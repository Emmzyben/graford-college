...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $courseOfStudy = $_POST["courseOfStudy"]; // Added this line

    // Check if a student with the same full name, matriculation number, and course of study already exists
    $checkQuery = "SELECT * FROM alumni WHERE fullName = ? AND matnumber = ? AND courseOfStudy = ?"; // Modified this line
    $checkStmt = $con->prepare($checkQuery);

    if ($checkStmt) {
        $checkStmt->bind_param("sss", $fullName, $matnumber, $courseOfStudy); // Modified this line
        $checkStmt->execute();
        $checkStmt->store_result();

        // If a matching record is found, display an error
        if ($checkStmt->num_rows > 0) {
            $errors[] = "Student with the same full name, matriculation number, and course of study already exists."; // Modified this line
        } else {
            ...
            $insertQuery = "INSERT INTO alumni (fullName, matnumber, graduationYear, courseOfStudy, picturePath) VALUES (?, ?, ?, ?, ?)"; // Modified this line
            $insertStmt = $con->prepare($insertQuery);

            if ($insertStmt) {
                // Bind parameters to the prepared statement
                $insertStmt->bind_param("sssss", $fullName, $matnumber, $graduationYear, $courseOfStudy, $picturePath); // Modified this line
  ...
  <div id="show">
        <div id="side">
            ...
            <ul>
                ...
                <li ><a href="overview.php">School Overview</a></li>
...
?>
    ...
    <?php
$query = 'SELECT id, fullName, matnumber, graduationYear, courseOfStudy, picturePath FROM alumni';
$result = mysqli_query($con, $query);

if ($result) {
    $alumni = mysqli_fetch_all($result, MYSQLI_ASSOC);
} else {
