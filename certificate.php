...
<p>Enter your unique certificate accreditation number below to verify</p>
</div>
...
<input type="text" name="AccreditationNumber" placeholder="Enter Certificate Number">
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $userInputAccreditationNumber = $_POST["AccreditationNumber"];

    // Use prepared statement to prevent SQL injection
    $sqlSelect = "SELECT fullName, matnumber, graduationYear, courseOfStudy, picturePath FROM alumni WHERE accreditationNumber = ?";
    
    // Check if the prepare statement was successful
    if ($stmtSelect = $conn->prepare($sqlSelect)) {
        $stmtSelect->bind_param("s", $userInputAccreditationNumber);
        $stmtSelect->execute();

        // Bind variables to the result set
        $stmtSelect->bind_result($fullName, $matnumber, $graduationYear, $courseOfStudy, $picturePath);
        ...
        if ($stmtSelect->fetch()) {
            ...
            // Display the details
            echo "<h3>Your certificate is valid, Here are your details</h3>";
            echo "<img src=\"$picturePath\" alt=\"Alumni Picture\">";
            echo "<p><b>Full Name:</b> $fullName</p>";
            echo "<p><b>Matriculation Number:</b> $matnumber</p>";
            echo "<p><b>Graduation Year:</b> $graduationYear</p>";
            echo "<p><b>Course of Study:</b> $courseOfStudy</p>";
        } else {
